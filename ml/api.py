import base64, io, joblib, hashlib, numpy as np
...
def predict(p: Payload):
    ...
    if usable:
        findings = [
            'Brain structure boundaries successfully isolated.' if not is_demo_sample else 'Demonstration scan signature verified.',
            'Percentile contrast stretching applied to normalize scanner variations.' if not is_demo_sample else 'Reference characteristics active.',
    ...
    return {
        ...
        'quality': {
            'label': quality_label,
            'comment': quality_comment if (not usable or is_demo_sample) else 'MRI cropped, squared, contrast-normalized, and resized to 96×96.'
        },
        'prediction': {
            'label': top['label'],
        ...
        'probabilities': items,
        'location': 'Not estimated by this classifier' if usable else 'Estimation disabled due to invalid input',
        'findings': findings,
